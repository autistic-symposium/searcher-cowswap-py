...
class SpreadSolverApi():
    ...
    ###########################
    @property
    def orders_surplus(self) -> dict:
        """Access to surplus for all paths."""

        return self.__surplus_data
    ...
    def _run_two_leg_trade_one_path(self, order, amms, simulation=False) -> dict:
        ...
        for amm_token, amm_data in amms.items():
            ...
            first_leg = solution_first_leg['amm_sell_token'] + solution_first_leg['amm_buy_token']

            solution_second_leg['amm_buy_token'] = second_leg_data['buy_token']
            solution_second_leg['amm_sell_token'] = second_leg_data['sell_token']
            second_leg = solution_second_leg['amm_sell_token'] + solution_second_leg['amm_buy_token']

            this_amms.update(
                {
                    first_leg: solution_first_leg,
                    second_leg: solution_second_leg
                })
    ...
    def _run_two_leg_trade_multiple_paths(self, order, amms) -> dict:
        ...
        path1, path2 = self._get_surplus_rank()
    ...
    def _get_surplus_rank(self) -> tuple:
