...
def nelder_mead_simplex_optimization(equation, boundary, initial_guess=None) -> float:
    """
        Run a simple form of the Nelder-Mead optimization solver, that 
        minimizes a scalar function of one or more variables.
    """

    # If initial guess (x0) is None, set to the middle
    initial_guess = initial_guess or float(div((boundary[1] - boundary[0]), 2))
